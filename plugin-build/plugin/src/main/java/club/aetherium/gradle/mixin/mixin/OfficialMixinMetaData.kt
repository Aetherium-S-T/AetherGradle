package club.aetherium.gradle.mixin.mixin

import com.google.gson.GsonBuilder
import com.google.gson.JsonObject
import com.google.gson.JsonParser
import club.aetherium.gradle.mixin.MixinRemapExtension
import club.aetherium.gradle.utils.forEachInZip
    ...
    override fun writeExtra(fs: FileSystem) {
        if (!parent.noRefmap.contains("BaseMixin")) {
            for ((name, json) in refmaps) {
                parent.logger.info("[AetherGradle/MixinMetaData] Writing refmap $name")
        ...
        for ((name, json) in mixinJsons) {
            parent.logger.info("[AetherGradle/MixinMetaData] Writing mixin config $name")
